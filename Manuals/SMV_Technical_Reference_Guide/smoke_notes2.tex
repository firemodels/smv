...
\author[1,2]{Glenn Forney}

\affil[1]{National Institute of Standards and Technology\\Gaithersburg, Maryland, USA}
\affil[2]{e-mail: glenn.forney@nist.gov}
...
This note discusses some of the physics and associated numerical algorithms used by Smokeview to
...
Approximations are required to display smoke and fire at interactive frame rates.
The primary approximation is to take advantage of the low albedo character of
smoke allowing one to either simplify or eliminate scattering terms in the RTE.
...
Realistic visualization of fire are important
...
discretized by converting integrals into a sum. Figure
...
\end{center}
\caption{Planes are placed within the entire solution domain.  The planes are equally
spaced and oriented perpendicular to the line of sight.}
...
domain resulting in a polygon which is triangulated
using a  2D coordinate system in the plane of this polygon.}
\label{fig:smoketriangulate}
\end{figure}

\item Coordinates $(a_i,b_i)$ are determined for each polygonal vertex, $p_i$ in
terms of $\vvec{s}$ and $\vvec{t}$ using
...
If $\vvec{s}$ and $\vvec{t}$ are thought of as two axese, then the coordinates $a_i$ and $b_i$ may be
interpreted as the distance from $\vvec{s}$ and $\vvec{t}$ respectively.
