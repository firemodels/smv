    ...
    for(i = 0; i < ngeominfoptrs; i++){
      ...
      for(itime = 0; itime < 2; itime++){
        ...
        if(itime==1&&geomi->geomtype==GEOM_ISO){
          ...
          if(use_tload_begin==1&&global_times[iglobal_times]<global_scase.tload_begin)continue;
          if(use_tload_end==1&&global_times[iglobal_times]>global_scase.tload_end)continue;
        }
