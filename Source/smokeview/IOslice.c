...
void UpdateSliceFilenum(void){
  ...
  for(ii=0;ii<nslice_loaded;ii++){
    ...
    sd = sliceinfo+i;
    if(sd->display==0||islicetype!=sd->slicetype2)continue;
    slicefilenum=i;
    break;
  }
}
  ...
  for(i = 0; i < sd->nslicei; i++){
    ...
    for(j = 0; j < sd->nslicej; j++){
      ...
      for(k = 0; k < sd->nslicek; k++){
        ...
        if(sd->slicetype1 == SLICE_CELL_CENTER &&
          ((k == 0 && sd->nslicek != 1) || (j == 0 && sd->nslicej != 1) || (i == 0 && sd->nslicei != 1)))continue;
        if(show_slice_in_obst == ONLY_IN_GAS){
          if(sd->slicetype1 != SLICE_CELL_CENTER&& iblank_node != NULL&&iblank_node[IJKNODE(sd->is1 + i, sd->js1 + j, sd->ks1 + k)] == SOLID)continue;
          if(sd->slicetype1 == SLICE_CELL_CENTER&& iblank_cell != NULL&&iblank_cell[IJKCELL(sd->is1 + i - 1, sd->js1 + j - 1, sd->ks1 + k - 1)] == EMBED_YES)continue;
        }
        slice_mask0[n] = 1;
      }
    }
...
void UpdateSliceBounds(void){
  ...
  for(i=0;i<nslicebounds;i++){
    ...
    for(jj=0;jj<nslice_loaded;jj++){
      ...
      slicej = sliceinfo + j;
      if(slicej->slicetype2!=i)continue;
...
void UpdateAllSliceColors(int slicetype, int *errorcode){
  ...
  *errorcode=0;
  ...
  for(ii=0;ii<nslice_loaded;ii++){
    i = slice_loaded_list[ii];
    sd = sliceinfo + i;
    if(sd->slicetype2!=slicetype)continue;
...
int SliceCompare( const void *arg1, const void *arg2 ){
  ...
  if(slicei->mesh_type>slicej->mesh_type)return 1;
  if(slicei->slicetype1<slicej->slicetype1)return -1;
  if(slicei->slicetype1>slicej->slicetype1)return 1;
...
int NewMultiSlice(slicedata *sdold,slicedata *sd){
  ...
  // convert from physical to scaled units using xyzmaxdiff
    ...
    if(strcmp(sd->label.shortlabel,sdold->label.shortlabel)!=0
      ...
      ||sd->slicetype1!=sdold->slicetype1
        ){
      return 1;
    }
  }
    ...
    if(strcmp(sd->label.shortlabel,sdold->label.shortlabel)!=0
      ||sd->mesh_type!=sdold->mesh_type||sd->slicetype1!=sdold->slicetype1
        ){
      return 1;
    }
  }
...
void UpdateFedinfo(void){
  ...
  for(i = 0; i < nsliceinfo; i++){
    ...
    fedi->loaded = 0;
    fedi->display = 0;
    if(slicei->slicetype1 != SLICE_CELL_CENTER&&strcmp(slicei->label.longlabel, "CARBON DIOXIDE VOLUME FRACTION") != 0)continue;
    if(slicei->slicetype1 == SLICE_CELL_CENTER&&strcmp(slicei->label.longlabel, "CARBON DIOXIDE VOLUME FRACTION(cell centered)") != 0)continue;
    fedi->co2_index = i;
    for(j = 0; j < nsliceinfo; j++){
      slicedata *slicej;

      slicej = sliceinfo + j;
      if(slicei->slicetype1 != SLICE_CELL_CENTER&&strcmp(slicej->label.longlabel, "CARBON MONOXIDE VOLUME FRACTION") != 0)continue;
      if(slicei->slicetype1 == SLICE_CELL_CENTER&&strcmp(slicej->label.longlabel, "CARBON MONOXIDE VOLUME FRACTION(cell centered)") != 0)continue;
    ...
    for(j = 0; j < nsliceinfo; j++){
      ...
      slicej = sliceinfo + j;
      if(slicei->slicetype1 != SLICE_CELL_CENTER&&strcmp(slicej->label.longlabel, "OXYGEN VOLUME FRACTION") != 0)continue;
      if(slicei->slicetype1 == SLICE_CELL_CENTER&&strcmp(slicej->label.longlabel, "OXYGEN VOLUME FRACTION(cell centered)") != 0)continue;
      if(slicei->blocknumber != slicej->blocknumber)continue;
      if(slicei->is1 != slicej->is1 || slicei->is2 != slicej->is2)continue;
      if(slicei->js1 != slicej->js1 || slicei->js2 != slicej->js2)continue;
      if(slicei->ks1 != slicej->ks1 || slicei->ks2 != slicej->ks2)continue; // skip if not the same size and in the same mesh
...
void UpdateSliceDirCount(void){
  ...
  for(i = 0; i < nmultisliceinfo; i++){
    ...
    for(j = 0; j < nmultisliceinfo; j++){
      ...
      if((slicej->slicetype1 == SLICE_CELL_CENTER&&slicei->slicetype1 != SLICE_CELL_CENTER) ||
        (slicej->slicetype1 != SLICE_CELL_CENTER&&slicei->slicetype1 == SLICE_CELL_CENTER))continue;
...
void GetSliceParams(void){
  ...
  if(nsliceinfo>0){
...

    {
      ...
      mslicei->mslicetype2=sd->slicetype2;//check  'type'
      for(i=1;i<nsliceinfo;i++){
        slicedata *sdold;
...
void UpdateVSlices(void){
  ...
  for(i=0;i<nsliceinfo;i++){
    ...
    vd->iw=-1;
    vd->ival=i;
    vd->vslicetype2=sdi->slicetype2;
    vd->vslicetype1=sdi->slicetype1;
    if(vd->vslicetype1==SLICE_CELL_CENTER){
      for(j=0;j<nsliceinfo;j++){
        slicedata *sdj;

        sdj = sliceinfo+j;
        if(sdj->slicetype1!=SLICE_CELL_CENTER)continue;
  ...
  if(nvsliceinfo>0){
    ...
    mvslicei->mvslicetype2=sliceinfo[vsd->ival].slicetype2;
    for(i=1;i<nvsliceinfo;i++){
      slicedata *sd, *sdold;
      vslicedata *vsdold;
      ...
      if(NewMultiSlice(sdold,sd)==1){
        ...
        mvslicei++;
        mvslicei->nvslices=0;
        mvslicei-> mvslicetype2=sd->slicetype2;
        mvslicei->ivslices=NULL;
        NewMemory((void **)&mvslicei->ivslices,sizeof(int)*nvsliceinfo);
      }
      mvslicei->nvslices++;
...
void UpdateVSliceTypes(void){
  ...
  for(i=0;i<nvsliceinfo;i++){
    vd = vsliceinfo+i;
    vd->vslicetype2= GetVSliceType(vd);
  }
}
...
void GetSliceDataBounds(slicedata *sd, float *pmin, float *pmax){
...

  {
    ...
    if(sd->slicetype1 == SLICE_CELL_CENTER) strcpy(slicelabel, "cell");
...
void DrawSliceFrame(){
  ...
  for(ii=0;ii<nslice_loaded;ii++){
    ...
    if(sd->display==0){
      ...
      if(sd->slicetype1==SLICE_NODE_CENTER&&show_node_slices_and_vectors==0)continue;
      if(sd->slicetype1==SLICE_CELL_CENTER||sd->slicetype1==SLICE_FACE_CENTER){
    ...
    if(orien==1&&sd->slicetype1==SLICE_NODE_CENTER){
      if(usetexturebar!=0){
...
void DrawVSliceFrame(void){
  ...
  for(i=0;i<nvsliceinfo;i++){
    ...
    vd = vsliceinfo + i;
    if(vd->loaded==0||vd->display==0||sliceinfo[vd->ival].slicetype2!=islicetype)continue;
    val = vd->val;
...
#define VAL w
    ...
    if(vd->vslicetype1==SLICE_TERRAIN){
      DrawVVolSliceTerrain(vd);
    }
    else if(vd->vslicetype1==SLICE_CELL_CENTER){
