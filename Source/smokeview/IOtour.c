...
void HermiteOther(float t, keyframe *kf1, keyframe *kf2, pathdata *pj){
  ...
  p0 = kf1->az_path;
  p1 = kf2->az_path;
  m0 = kf1->az_tangent_right;
  m1 = kf2->az_tangent_left;
  pj->az_path = HERMVAL();

  p0 = kf1->nodeval.elev_path;
  p1 = kf2->nodeval.elev_path;
  m0 = kf1->elev_tangent_right;
  m1 = kf2->elev_tangent_left;
   ...
   // construct distance array based on number of points in each interval
    ...
    for(j=0;j<tour_ntimes;j++){
      ...
      if(kf1->viewtype==REL_VIEW||kf2->viewtype==REL_VIEW){
        ...
        VEC3ADD(tour_view, xyz,dxyz2);
      }
      else{
        float dxyz[3], denom;

        VEC3DIFF(dxyz,xyz_view, xyz);
        denom = 10.0*NORM3(dxyz);
        if(denom == 0.0)denom = 1.0;
        dxyz[0] /= denom;
        dxyz[1] /= denom;
        dxyz[2] /= denom;
        VEC3ADD(tour_view, xyz,dxyz);
      }
...
keyframe *AddFrame(keyframe *last_frame, float time_local, float *xyz, float key_az_path, float elev_path, int viewtype, float view[3]){
...
void InitCircularTour(tourdata *touri, int nkeyframes, int option){
  ...
  for(j=0;j<nkeyframes;j++){
    ...
    addedframe=AddFrame(thisframe, key_time, key_xyz, key_az_path, elev_path, viewtype, key_view);
