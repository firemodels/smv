...
extern "C" int GLUIGetChopMin(int type, char *label, int *set_chopmin, float *chopmin){
  switch(type){
    ...
    case BOUND_PLOT3D:
      ...
      break;
    case BOUND_SLICE:
      if(nsliceinfo>0)return sliceboundsCPP.get_chopmin(label, set_chopmin, chopmin);
      break;
    default:
      assert(FFALSE);
      break;
...
extern "C" int GLUIGetChopMax(int type, char *label, int *set_chopmax, float *chopmax){
  switch(type){
    ...
    case BOUND_PLOT3D:
      ...
      break;
    case BOUND_SLICE:
      if(nsliceinfo>0)return sliceboundsCPP.get_chopmax(label, set_chopmax, chopmax);
      break;
    default:
      assert(FFALSE);
      break;
...
void BoundBoundCB(int var){
  ...
  case READ_TEST:
    if(MakeFile(frametest_filename, read_buffer_size) == 1){
      ...
      bufferinfo = File2Buffer(frametest_filename, bufferinfo, ALLDATA_OFFSET, ALLDATA_NVALS, nframe_threads, &nread);
