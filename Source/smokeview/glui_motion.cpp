...
void SubRenderRolloutCB(int var){
  ToggleRollout(subrenderprocinfo, nsubrenderprocinfo, var);
}

/* ------------------ MVRRolloutCB ------------------------ */

void MVRRolloutCB(int var){
  ToggleRollout(mvrprocinfo, nmvrprocinfo, var);
}
...
extern "C" void ViewpointCB(int var){
  ...
  case LIST_VIEW:
    ...
    if(current_script_command==NULL&&update_startup_view>0)AdjustY(camera_current);
    if(current_script_command!=NULL&&update_viewpoint_script>0)AdjustY(camera_current);
#else
    AdjustY(camera_current);
#endif
    break;
