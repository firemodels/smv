  ...
  if(ndeviceinfo>0){
    ...
    if(GetNumActiveDevices()>0||isZoneFireModel==1){
      ...
      if(ntreedeviceinfo>0){
        ...
        CHECKBOX_vis_xtree = glui_device->add_checkbox_to_panel(ROLLOUT_trees, "Show x", &vis_xtree);
        CHECKBOX_vis_ytree = glui_device->add_checkbox_to_panel(ROLLOUT_trees, "Show y", &vis_ytree);
        CHECKBOX_vis_ztree = glui_device->add_checkbox_to_panel(ROLLOUT_trees, "Show z", &vis_ztree);
      }
