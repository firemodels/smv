...
extern "C" void GLUIShooterSetup(int main_window){
  ...
  glui_shooter = GLUI_Master.create_glui("Particle tracking",0,dialogX0,dialogY0);
  glui_shooter->hide();

  ROLLOUT_shooter_frame = glui_shooter->add_rollout("Starting locations/velocities", true, START_SHOOTER_ROLLOUT, ShooterRolloutCB);
  TOGGLE_ROLLOUT(shooterprocinfo, nshooterprocinfo, ROLLOUT_shooter_frame, START_SHOOTER_ROLLOUT, glui_shooter);

  PANEL_shooter_frameE=glui_shooter->add_panel_to_panel(ROLLOUT_shooter_frame,"Positions",false);
  ...
  PANEL_shooter_frameF=glui_shooter->add_panel_to_panel(ROLLOUT_shooter_frame,"Velocities");
  ...
  ROLLOUT_shooter_velocity = glui_shooter->add_rollout("Background velocity field", false, BACKGROUND_SHOOTER_ROLLOUT, ShooterRolloutCB);
  ...
  RADIOBUTTON_shooter_1=glui_shooter->add_radiobutton_to_group(RADIO_shooter_vel_type,"Power law");
...
//      sprintf(label,"%f",plot3dtimelist[i]);
  ...
  PANEL_shooter_frameD=glui_shooter->add_panel_to_panel(ROLLOUT_shooter_velocity,"Power law");
  SPINNER_shooter_u0=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameD,"reference velocity, U0 (m/s)",GLUI_SPINNER_FLOAT,&shooter_u0,SHOOTER_VEL,ShooterCB);
  SPINNER_shooter_z0=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameD,"reference elevation, Z0 (m)",GLUI_SPINNER_FLOAT,&shooter_z0,SHOOTER_VEL,ShooterCB);
  SPINNER_shooter_p=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameD,"decay, p",GLUI_SPINNER_FLOAT,&shooter_p,SHOOTER_VEL,ShooterCB);
  SPINNER_shooter_veldir=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameD,"velocity direction (deg)",GLUI_SPINNER_FLOAT,&shooter_veldir,SHOOTER_VEL,ShooterCB);
  ...
  CHECKBOX_shooter_1=glui_shooter->add_checkbox_to_panel(PANEL_shooter_frameG,"Show particles",&visShooter,SHOOTER_SHOW,ShooterCB);
  CHECKBOX_shooter_2=glui_shooter->add_checkbox_to_panel(PANEL_shooter_frameG,"Update continuously",&shooter_cont_update);
  CHECKBOX_shooter_3=glui_shooter->add_checkbox_to_panel(PANEL_shooter_frameG,"Show only first frame",&shooter_firstframe,SHOOTER_FIRSTFRAME,ShooterCB);
  SPINNER_shooter_v_inf=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameG,"terminal velocity",GLUI_SPINNER_FLOAT,&shooter_v_inf,SHOOTER_TERMINAL_VEL,ShooterCB);
  BUTTON_shooter_1=glui_shooter->add_button_to_panel(PANEL_shooter_frameG,"Compute tracks",SHOOTER_APPLY,ShooterCB);

  SPINNER_shooter_1=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameH,"Particle size",GLUI_SPINNER_FLOAT,&shooterpointsize);
  SPINNER_shooter_nparts=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameH,"number of particles",GLUI_SPINNER_INT,&shooter_nparts,SHOOTER_NPARTS,ShooterCB);

  SPINNER_shooter_fps=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameH,"frames per second",GLUI_SPINNER_INT,&shooter_fps,SHOOTER_FPS,ShooterCB);

  SPINNER_shooter_duration=glui_shooter->add_spinner_to_panel(PANEL_shooter_frameH,"duration (s)",GLUI_SPINNER_FLOAT,&shooter_duration,SHOOTER_DURATION,ShooterCB);
