...
GLUI_Spinner *SPINNER_smoke3d_delta_perp = NULL;
#endif
#ifdef pp_GPU
...
GLUI_Checkbox *CHECKBOX_compute_smoke = NULL;
...
#define HRRPUV_CUTOFF_MAX (hrrpuv_max_smv-0.01)
  ...
  if(nsmoke3dinfo>0){
...
#ifdef pp_GPUSMOKE
    ROLLOUT_smoketest = glui_3dsmoke->add_rollout_to_panel(ROLLOUT_slices,_("Smoke test"),false);
    PANEL_smokealg = glui_3dsmoke->add_panel_to_panel(ROLLOUT_smoketest, _("smoke drawing algorithm"));
    ...
    SPINNER_smoke3d_delta_par = glui_3dsmoke->add_spinner_to_panel(PANEL_smokealg, _("Parallel delta"), GLUI_SPINNER_FLOAT, &smoke3d_delta_par, SMOKE_DELTA, Smoke3dCB);
    SPINNER_smoke3d_delta_perp = glui_3dsmoke->add_spinner_to_panel(PANEL_smokealg, _("Perpindicular delta"), GLUI_SPINNER_FLOAT, &smoke3d_delta_perp, SMOKE_DELTA, Smoke3dCB);
  ...
  case SMOKE_NEW:
    if(use_newsmoke == SMOKE3D_ORIG || use_newsmoke == SMOKE3D_NEW){
      if(compute_smoke3d_planes!=1){
        compute_smoke3d_planes = 1;
        CHECKBOX_compute_smoke->set_int_val(compute_smoke3d_planes);
      }
      if(plane_single!=0){
        plane_single = 0;
    ...
    break;
  case SMOKE_DELTA:
    if(smoke3d_delta_perp<=0.0){
      smoke3d_delta_perp = 0.1;
      SPINNER_smoke3d_delta_perp->set_float_val(smoke3d_delta_perp);
    }
