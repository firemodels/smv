...
! why do we care?  if a vertex is on the exterior (connected to a blockage or mesh boundary for example)
!                  we won't move it or delete it
...
SUBROUTINE DECIMATE(VERTS, N_VERTS, TRIANGLES, N_TRIANGLES, MESH_BOUNDS, DELTA)
...
INTEGER, INTENT(INOUT) :: N_VERTS, N_TRIANGLES
REAL(EB), INTENT(INOUT), DIMENSION(3*N_VERTS), TARGET :: VERTS
INTEGER, INTENT(INOUT), DIMENSION(3*N_TRIANGLES), TARGET :: TRIANGLES
REAL(EB), INTENT(IN) :: DELTA
REAL(EB), DIMENSION(6), INTENT(IN) :: MESH_BOUNDS

INTEGER, PARAMETER :: V_MERGED=-1, V_DISCARD=0, V_ORIGINAL=1

INTEGER, DIMENSION(N_VERTS) :: VERT_STATE, VERT_MAP, VERT_TYPE
...
DO WHILE (HAVE_SMALL .AND. ITER<MAX_ITER) ! iterate until no further changes are made (or 10 times whichever comes first)
   ...
   VERT_STATE(1:N_VERTS) = V_ORIGINAL
   ...
   CALL GET_VERTTYPE(N_VERTS, TRIANGLES, N_TRIANGLES, VERT_TYPE)

! combine vertices that are close together

   TRI_LOOP: DO I = 1, N_TRIANGLES
      TRI_I=>TRIANGLES(3*I-2:3*I)
   ...
   ITO = 0
   DO IFROM = 1, N_TRIANGLES
      TRI_FROM=>TRIANGLES(3*IFROM-2:3*IFROM)
      IF (TRI_FROM(1)/=TRI_FROM(2) .AND. TRI_FROM(1)/=TRI_FROM(3) .AND. TRI_FROM(2)/=TRI_FROM(3)) THEN
        ...
        IF (TRI_NEW(1)/=0 .AND. TRI_NEW(2)/=0 .AND. TRI_NEW(3)/=0) THEN
          ITO=ITO+1
          TRI_TO=>TRIANGLES(3*ITO-2:3*ITO)
          TRI_TO(1)=TRI_NEW(1)
          TRI_TO(2)=TRI_NEW(2)
          TRI_TO(3)=TRI_NEW(3)
        ENDIF
...
SUBROUTINE GEOMCLIP(VERTS, N_VERTS, TRIANGLES, N_TRIANGLES, XB)
INTEGER, INTENT(INOUT) :: N_VERTS, N_TRIANGLES
REAL(EB), INTENT(INOUT), DIMENSION(3*N_VERTS), TARGET :: VERTS
INTEGER, INTENT(INOUT), DIMENSION(3*N_TRIANGLES), TARGET :: TRIANGLES
...
INTEGER, POINTER, DIMENSION(:) :: TRII
REAL(EB), POINTER, DIMENSION(:) :: V1, V2, V3
INTEGER, DIMENSION(N_VERTS) :: HAVE_VERT, VERT_MAP
INTEGER :: NT, NV

! find triangles that are not within the box defined by XB

HAVE_VERT(1:N_VERTS) = 0
NT = 0
DO I = 1, N_TRIANGLES
  TRII(1:3) = TRIANGLES(3*I-2:3*I)
  V1(1:3) = VERTS(3*TRII(1)-2:3*TRII(1))
  V2(1:3) = VERTS(3*TRII(2)-2:3*TRII(2))
  V3(1:3) = VERTS(3*TRII(3)-2:3*TRII(3))
  IF (BOXTRIANGLE(XB,V1,V2,V3)==1) THEN
    NT = NT + 1
    IF(NT /= I)TRIANGLES(3*NT-2:3*NT) = TRII(1:3)
    HAVE_VERT(TRII(1))=1
    HAVE_VERT(TRII(2))=1
    HAVE_VERT(TRII(3))=1
  ENDIF
