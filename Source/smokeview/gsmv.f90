...
SUBROUTINE GEOM_OFFSET(VERTS_IN, NVERTS_IN, TRIS_IN, NTRIS_IN, MESH, OFFSET, VERTS_OUT, NVERTS_OUT, TRIS_OUT, NTRIS_OUT)
...
REAL(EB), INTENT(OUT), POINTER, DIMENSION(:) :: VERTS_OUT
INTEGER, INTENT(OUT), POINTER, DIMENSION(:) :: TRIS_OUT

INTEGER :: I, I_OUT
REAL(EB), POINTER, DIMENSION(:) :: V1, V2, V3, VIN, VOUT, CV
REAL(EB), TARGET :: CVERTS(NTRIS_IN)
REAL(EB) :: CVPOFFSET(3)
...
DO I = 1, NTRIS_IN
   ...
   CV(1:3) => CVERTS(3*I-2:3*I)
   CV = (V1+V2+V3)/3.0_EB
   CVPOFFSET = CV + OFFSET
   KEEP_TRI(I) = 0
   IF(CVPOFFSET(1)<MESH(1).OR.CVPOFFSET(1)>MESH(2))CYCLE
   IF(CVPOFFSET(2)<MESH(3).OR.CVPOFFSET(2)>MESH(4))CYCLE
   IF(CVPOFFSET(3)<MESH(5).OR.CVPOFFSET(3)>MESH(6))CYCLE
   KEEP_TRI(I) = 1
   KEEP_VERT(TRI(1)) = 1
   KEEP_VERT(TRI(2)) = 1
   KEEP_VERT(TRI(3)) = 1
...
DO I = 1, NVERTS_IN
   IF(KEEP_VERT(I) .EQ. 1)THEN
      ...
      VOUT => VERTS_IN(3*I_OUT-2:3*I_OUT)
      VOUT = VIN + OFFSET
   ENDIF
END DO
