...
void UsageLua(char *prog,int option){
  char buffer[1000];
...
void ParseCommandlineLua(int argc, char **argv){
  int i, len_casename;
  int iarg;
  size_t len_memory;
  char *argi;
  ...
  for(i = 1; i < argc; i++){
    ...
    if(strncmp(argv[i], "-update_bounds", 14) == 0){
      ...
      UsageLua(argv[0],HELP_SUMMARY);
      exit(0);
    }
    else if(strncmp(argv[i], "-help_all", 9)==0){
      UsageLua(argv[0],HELP_ALL);
      exit(0);
    }
...
int RunLuaBranch(lua_State *L, int argc, char **argv) {
  ...
  if(show_help==1){
    UsageLua("smokeview",HELP_SUMMARY);
    return 1;
}
  if(show_version==1){
...
#endif
  ParseCommandlineLua(argc, argv_sv);
