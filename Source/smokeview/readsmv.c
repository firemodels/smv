...
int ReadINI2(char *inifile, int localfile){
  ...
  while(!feof(stream)){
    ...
    if(Match(buffer, "HISTOGRAM") == 1){
      fgets(buffer, 255, stream);
      sscanf(buffer, " %i %i %i %f",&histogram_type, &histogram_nbuckets, &histogram_outline, &histogram_width_factor);
      continue;
    }
...
/*  extern GLfloat iso_ambient[4], iso_specular[4], iso_shininess;*/
  ...
  fprintf(fileout, " %i %i %i %f\n", histogram_type, histogram_nbuckets, histogram_outline, histogram_width_factor);
