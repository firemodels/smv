...
#ifdef pp_SMOKEFRAME
  if(scase->smoke3dcoll.nsmoke3dinfo>0 && NewMemory((void **)&scase->smoke3d_buffer, 10*scase->smoke3dcoll.nsmoke3dinfo*MAXFILELEN) == 0)return 2;
#else
  ...
  */
    ...
    */
    if(MatchSMV(buffer, "HRRPUV_MINMAX") == 1){
      FGETS(buffer, 255, stream);
      sscanf(buffer, "%f %f", &global_hrrpuv_min, &global_hrrpuv_max);
      continue;
    }
    ...
    */
    if(MatchSMV(buffer, "TEMP_MINMAX") == 1){
      FGETS(buffer, 255, stream);
      sscanf(buffer, "%f %f", &global_temp_min, &global_temp_max);
      continue;
    }
