  ...
  while(!feof(stream)){
    ...
    if(Match(buffer, "GEOMSHOW") == 1){
      ...
      sscanf(buffer, " %i %i %i %i %i %i %f %f %i %i",
        &dummy, &dummy2, &show_faces_shaded, &show_faces_outline, &smooth_geom_normal, &show_cface_normals,
...
void WriteIni(int flag,char *filename){
  ...
  fprintf(fileout, " %i %i %i %i %i %i %f %f %i %i\n",
     0, 1, show_faces_shaded, show_faces_outline, smooth_geom_normal, show_cface_normals,
     geom_force_transparent, geom_transparency, geom_linewidth, use_geom_factors);
