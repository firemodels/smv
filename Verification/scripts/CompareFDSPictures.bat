...
cd %CURDIR%\..\..\..\fds\Manuals\
set MANDIR=%CD%

cd %MANDIR%\FDS_User_Guide\SCRIPT_FIGURES
set TODIR=%CD%

cd %CURDIR%\..\..\..\fds
set FDSREPODIR=%CD%

git describe --dirty --long > %CURDIR%\fdsrepo.txt
set /p FDSREPOREVISION=<%CURDIR%\fdsrepo.txt
...
for %%d in ( FDS_User_Guide FDS_Verification_Guide ) do (
if %%d == FDS_User_Guide         set FILELIST=%CURDIR%\user_filelist.txt
if %%d == FDS_Verification_Guide set FILELIST=%CURDIR%\veri_filelist.txt
...
echo ^<html^>
echo ^<head^>
echo ^<TITLE^> %FDSREPOREVISION% %date% %time% ^</TITLE^>
echo ^</HEAD^>
...
for %%d in (FDS_User_Guide FDS_Verification_Guide) do (
if %%d == FDS_User_Guide echo ^<a name="changeduser"^>
if %%d == FDS_Verification_Guide echo ^<a name="changedveri"^>

if %%d == FDS_User_Guide         echo ^<h2^>Changed User Images^</h2^>
if %%d == FDS_Verification_Guide echo ^<h2^>Changed Verification Images^</h2^>

if %%d == FDS_User_Guide         set FILELOOP=%CURDIR%\user_nonzerolist.txt 
if %%d == FDS_Verification_Guide set FILELOOP=%CURDIR%\veri_nonzerolist.txt 

if %%d == FDS_User_Guide         echo [Changed User Images]
if %%d == FDS_Verification_Guide echo [^<a href="#changeduser"^>Changed User Images^</a^>]
                                 echo [^<a href="#Unchangeduser"^>Unchanged User Images        ^</a^>]
if %%d == FDS_User_Guide         echo [^<a href="#changedveri"  ^>Changed Verification Images  ^</a^>]
if %%d == FDS_Verification_Guide echo [Changed Verification Images]
...
if %%d == FDS_User_Guide echo ^<a name="Unchangeduser"^>
if %%d == FDS_Verification_Guide echo ^<a name="UnchangedVeri"^>

if %%d == FDS_User_Guide         echo ^<h2^>Unchanged User Images^</h2^>
if %%d == FDS_Verification_Guide echo ^<h2^>Unchanged Verification Images^</h2^>

echo [^<a href="#changeduser"  ^>Changed User Images          ^</a^>]
if %%d == FDS_User_Guide         echo [Unchanged User Images]
if %%d == FDS_Verification_Guide echo [^<a href="#Unchangeduser"^>Unchanged User Images        ^</a^>]
                                 echo [^<a href="#changedveri"  ^>Changed Verification Images  ^</a^>]
if %%d == FDS_User_Guide         echo [^<a href="#Unchangedveri"^>Unchanged Verification Images^</a^>]
if %%d == FDS_Verification_Guide echo [Unchanged Verification Images]

if %%d == FDS_User_Guide         set FILELOOP=%CURDIR%\user_zerolist.txt 
if %%d == FDS_Verification_Guide set FILELOOP=%CURDIR%\veri_zerolist.txt 
